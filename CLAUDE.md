...
- **Editing pattern (no per-section pencils):** click an existing item directly to open a full edit modal, always available regardless of the toggle below. Separately, the top-corner pencil is a single ON/OFF toggle for "customize mode": while ON, section headers show a "+" add button and every section becomes drag-to-reposition and horizontally resizable (around the fixed center orb); while OFF, none of that is visible. Anything not editable this way lives on its own dedicated sidebar tab instead (e.g. full task management, Finance detail).
